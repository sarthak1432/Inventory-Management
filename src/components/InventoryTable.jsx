...
const MobileCard = React.memo(({ item, onEdit, onDelete, onEngage }) => {
  const status = getStockStatus(item.quantity);
  return (
    <div className="p-4 space-y-4 hover:bg-slate-50 transition-colors">
...
const DesktopRow = React.memo(({ item, onEdit, onDelete, onEngage }) => {
  const status = getStockStatus(item.quantity);
  return (
