...
const EngagementRow = React.memo(({ item, onReturn, onDelete }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white border border-slate-200 rounded-xl p-3 md:p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:border-indigo-200 transition-all group"
    >
      <div className="flex items-start md:items-center gap-2 md:gap-3 flex-grow w-full md:max-w-md">
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center font-black text-[10px] md:text-xs text-slate-400 uppercase shrink-0">
          {item.name.substring(0, 2)}
        </div>
        <div className="min-w-0 flex-grow">
          <div className="flex items-center justify-between md:justify-start gap-2 mb-0.5 md:mb-1">
            <h4 className="font-black text-slate-900 uppercase tracking-tight truncate text-xs md:text-sm">{item.name}</h4>
            <span className="px-1.5 py-0.5 bg-slate-100 text-[8px] md:text-[9px] font-bold text-slate-500 rounded-md uppercase tracking-wider shrink-0 capitalize">{item.department}</span>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-slate-400 text-[9px] md:text-[10px] font-bold uppercase tracking-wider">
            {item.receiver && (
              <div className="flex items-center gap-1 text-indigo-600 truncate">
                <UserCheck size={10} className="shrink-0 md:w-3 md:h-3" />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-6 w-full md:w-auto mt-2 md:mt-0 pt-2 md:pt-0 border-t border-slate-50 md:border-0 sm:justify-between">
        <div className="flex items-center justify-between sm:block w-full sm:w-auto">
          <p className="text-[8px] md:text-[9px] font-black text-slate-300 uppercase tracking-widest mb-0 sm:mb-0.5">Quantity</p>
          <p className="text-sm md:text-base font-black text-slate-900 leading-none">{item.quantity} <span className="text-[8px] md:text-[9px] text-slate-400">UNITS</span></p>
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto">
          {item.note && (
            <div className="hidden lg:block group/note relative">
              <div className="p-1.5 transition-colors text-slate-300 hover:text-indigo-400">
                <Info size={16} />
              </div>
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-48 p-2 bg-slate-900 text-white text-[9px] rounded-lg opacity-0 invisible group-hover/note:opacity-100 group-hover/note:visible transition-all z-20 font-medium">
                {item.note}
              </div>
            </div>
          )}
          ...
          {isReturnable ? (
            <button
              ...
              className="flex-1 sm:flex-none justify-center px-3 md:px-4 py-1.5 md:py-2 bg-slate-900 hover:bg-indigo-600 text-white rounded-lg transition-all text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none justify-center px-3 md:px-4 py-1.5 md:py-2 bg-white border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-500 hover:bg-red-50 rounded-lg transition-all text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5"
            >
              <Trash2 size={12} />
              <span>Remove</span>
            </button>
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-12 animate-fade-in px-1">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 pb-2 border-b border-slate-100">
        <div>
          <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Stocks Engaged</h3>
          <p className="text-slate-500 text-[10px] sm:text-xs md:text-sm font-medium mt-1">
        ...
        <button
          onClick={onEngage}
          className="inline-flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-slate-900 px-4 py-2 rounded-lg font-bold text-white text-[10px] md:text-[11px] uppercase tracking-widest transition-all shadow-md shadow-indigo-100 active:scale-95 group shrink-0 w-full sm:w-auto"
        >
